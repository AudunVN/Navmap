...
namespace FLInfocardIE.Util_Functions
{
    public class FLGameData
    {
        ...
        private string GetIDString(uint iIDS)
        {
            ...
            if (vDLLs.Count > iDLL)
            {
                ...
                if (hInstance != IntPtr.Zero)
                {
                    ...
                    if (len > 0)
                    {
                        return System.Text.Encoding.Unicode.GetString(bufName, 0, len);
                    }
        ...
        private IDS GetIDSParm(FLDataFile.Section section, string parmName)
        {
            IDS ids = new IDS();
            ids.stInfo = "";
            if (section.SettingExists(parmName))
            {
                ids.ids = section.GetSetting(parmName).UInt(0);
                ids.stInfo = GetIDString(ids.ids);
                if (ids.stInfo == null)
                    throw new Exception("ids_info not found " + ids.ids);
                ids.stInfo = ids.stInfo.Trim();
            }
            return ids;
        }
        ...
        private bool AddGameData(GameDataSet.HashListDataTable items, FLDataFile.Section section, string gameDataType, bool ignoreEntriesWithNoIds)
        {
            ...
            IDS stIDSName = GetIDSParm(section, "ids_name");
            if (stIDSName.stInfo == "")
                stIDSName = GetIDSParm(section, "strid_name");

            if (ignoreEntriesWithNoIds && stIDSName.stInfo == "")
                return false;
            ...
            IDS stIDSInfo = GetIDSParm(section, "ids_info");

            IDS stIDSInfo1 = GetIDSParm(section, "ids_info1");
            if (stIDSInfo1.stInfo == "")
                stIDSInfo1 = GetIDSParm(section, "ids_short_name");

            IDS stIDSInfo2 = GetIDSParm(section, "ids_info2");
            IDS stIDSInfo3 = GetIDSParm(section, "ids_info3");

            string keys = hash.ToString() + " 0x" + hash.ToString("X");

            items.AddHashListRow(hash, nickName, gameDataType, stIDSName.ids, stIDSInfo.ids, stIDSInfo1.ids, stIDSInfo2.ids, stIDSInfo3.ids, keys, stIDSName.stInfo, stIDSInfo.stInfo, stIDSInfo1.stInfo, stIDSInfo2.stInfo, stIDSInfo3.stInfo);
            return true;
        }
                ...
                if (section.sectionName.ToLowerInvariant() == "object")
                {
                    try
                    {
                        ...
                        if (section.SettingExists("base"))
                        {
                            ...
                            if (baseItem != null)
                            {
                                if (baseItem.IDSInfoTxt.Length == 0)
                                    baseItem.IDSInfoTxt = idsInfo;
                                if (baseItem.IDSInfo1Txt.Length == 0)
                                    baseItem.IDSInfo1Txt = idsInfo1;
                                if (baseItem.IDSInfo2Txt.Length == 0)
                                    baseItem.IDSInfo2Txt = "Postion: " + pos;
                            }
                        }
        ...
        private void LoadRoomData(GameDataSet dataStore, string baseNick, string baseIniPath)
        {
            ...
            foreach (FLDataFile.Section section in iniRoom.sections)
            {
                ...
                {
                    dataStore.HashList.AddHashListRow(hashU, locationNick, GAMEDATA_GEN, 0, 0, 0, 0, 0, keys, "", "", "", "", "");
                }
            }   
        }
                    ...
                    foreach (FLDataFile.Section section in ini.sections)
                    {
                        foreach (FLDataFile.Setting e in section.settings)
                        {
                                ...
                                || st == "msg_id_prefix" || st == "npc")
                            {
                                ...
                                {
                                    string keys = hashU.ToString() + " 0x" + hashU.ToString("X");
                                    dataStore.HashList.AddHashListRow(hashU, nickName, GAMEDATA_GEN, 0, 0, 0, 0, 0, keys, "", "", "", "", "");
                                }
                            }
                        ...
                        try
                        {
                            ...
                            if (itemRecord != null)
                            {
                                if (setting.NumValues() > 0)
                                    itemRecord.IDSNameTxt = setting.Str(0);
                            }
                        }
                        catch (Exception ex)
                        {
                        ...
                        try
                        {
                            ...
                            if (itemRecord != null)
                            {
                                if (setting.NumValues() > 1)
                                    itemRecord.IDSNameTxt = setting.Str(1);
